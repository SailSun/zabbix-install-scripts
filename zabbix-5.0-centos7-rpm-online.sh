...
## Install Basic Rpms
yum -y install vim wget net-tools net-snmp-utils fping httpd-tools yum-utils

## Install PostgreSQL
yum -y install https://download.postgresql.org/pub/repos/yum/reporpms/EL-$(rpm -E %{centos})-x86_64/pgdg-redhat-repo-latest.noarch.rpm
...
## Config Zabbix Web
zabbixConf="/etc/opt/rh/rh-nginx116/nginx/conf.d/zabbix.conf"
nginxConf="/etc/opt/rh/rh-nginx116/nginx/nginx.conf"
cp $zabbixConf{,.bak}
cp $nginxConf{,.bak}
